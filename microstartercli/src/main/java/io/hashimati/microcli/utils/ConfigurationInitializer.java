...
//                        (templatesService.getMicrometersTemplates().get(STATSD_yml)));
        ...
        if(!projectInfo.getFeatures().contains("retry"))
        {
            MicronautProjectValidator.addDependency(workingPath, features.get("retry"));

        }
        if(!projectInfo.getFeatures().contains("micronaut-validation"))
        {
            MicronautProjectValidator.addDependency(workingPath, features.get("micronaut-validation"));
