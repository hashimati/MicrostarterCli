...
public class CreateRabbitMQClientCommand implements Callable<Integer> {
    ...
    public Integer call() throws Exception {
        ...
        if(!configurationInfo
                .getProjectInfo().getFeatures().contains("rabbitmq")){

            TemplatesService templatesService = new TemplatesService();
            projectInfo.getFeatures().add("rabbitmq");
            try {
                MicronautProjectValidator.addDependency(path,FeaturesFactory.features(configurationInfo.getProjectInfo()).get("rabbitmq"));
        ...
        String className = PromptGui.inputText("className", "Enter the class name: ", "RabbitMQClient").getInput();
